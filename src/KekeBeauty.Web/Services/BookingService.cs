...
{
    public async Task<List<Booking>> List(long account, string state = "", DateOnly? from = null, DateOnly? to = null, int page = 1, int pageSize = 20)
    {
        await using var command = db.CreateCommand("""
            ...
            WHERE EXISTS(SELECT 1 FROM kb.compte WHERE compte_id=$1 AND etat='actif') AND ($2='' OR r.etat=$2)
              AND ($3::date IS NULL OR (SELECT min(debut) FROM kb.ligne_rdv l WHERE l.rdv_id=r.rdv_id)>=$3::date)
              AND ($4::date IS NULL OR (SELECT min(debut) FROM kb.ligne_rdv l WHERE l.rdv_id=r.rdv_id)<$4::date+1)
              AND (r.client_id=$1 OR EXISTS(SELECT 1 FROM kb.gerant g JOIN kb.permission_gestion pg USING(gerant_id)
                WHERE g.compte_id=$1 AND pg.etablissement_id=e.etablissement_id AND pg.permission_code='rdv_decider'))
            ORDER BY r.cree_le DESC,r.rdv_id DESC LIMIT $5 OFFSET $6
            """);
        command.Parameters.AddWithValue(account);
        command.Parameters.AddWithValue(state);
        command.Parameters.AddWithValue(NpgsqlTypes.NpgsqlDbType.Date,(object?)from ?? DBNull.Value);
        command.Parameters.AddWithValue(NpgsqlTypes.NpgsqlDbType.Date,(object?)to ?? DBNull.Value);
    ...
    public async Task<List<BookingCount>> Counts(long account)
    {
        await using var command = db.CreateCommand("""
            SELECT r.etat,count(*) FROM kb.rendez_vous r JOIN kb.politique_rdv p USING(politique_id)
            WHERE EXISTS(SELECT 1 FROM kb.gerant g JOIN kb.permission_gestion pg USING(gerant_id)
              WHERE g.compte_id=$1 AND pg.etablissement_id=p.etablissement_id AND pg.permission_code='rdv_decider')
            GROUP BY r.etat ORDER BY r.etat
            """);
        command.Parameters.AddWithValue(account); List<BookingCount> result=[];
        await using var reader=await command.ExecuteReaderAsync();
        while(await reader.ReadAsync()) result.Add(new(reader.GetString(0),reader.GetInt64(1)));
        return result;
    }
    ...
    public async Task Act(long booking, long account, string action, string key, string reason)
    {
        var sql = action switch
        {
            ...
            "termine" => "SELECT kb.cloturer_rdv($1,$2,'termine')",
            "absent" => "SELECT kb.cloturer_rdv($1,$2,'absent')",
    ...
    public async Task MarkNotificationsRead(long account)
    {
        await using var command = db.CreateCommand("UPDATE kb.notification SET lu_le=COALESCE(lu_le,now()) WHERE destinataire_id=$1 AND canal='in_app'");
        command.Parameters.AddWithValue(account); await command.ExecuteNonQueryAsync();
    }
